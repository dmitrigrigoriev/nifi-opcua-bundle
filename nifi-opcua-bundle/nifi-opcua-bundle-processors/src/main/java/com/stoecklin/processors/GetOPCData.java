...
public class GetOPCData extends AbstractProcessor {
    ...
    public static final PropertyDescriptor DATA_FORMAT = new PropertyDescriptor
            ...
            .description("The format the data should be in, either TEMPUS or CSV")
            .required(true)
            .allowableValues("TEMPUS","CSV")
            .defaultValue("TEMPUS")
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .build();
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        if ( flowFile == null ) {
            logger.error("GetOPCData.onTrigger(): Flow File is null for session: " + session.toString());
            return;
        }
        ...
        session.read(flowFile, new InputStreamCallback() {
            ...
            public void process(InputStream in) throws IOException {
                ...
                try{
        			...
        			e.printStackTrace();
        			logger.error("GetOPCData.onTrigger(): Failed to read");
        		}
            }
        });
        ...
        if(opcUAService.updateSession()){
        	logger.debug("GetOPCData.onTrigger(): Session update complete");
        }else {
        ...
        if (context.getProperty(DATA_FORMAT).getValue().toString().equals("TEMPUS")){
        ...
        try {
        	...
        	logger.error("GetOPCData.onTrigger(): Unable to process: " + ex.getMessage());
        	session.transfer(flowFile, FAILURE);
        }
    }
